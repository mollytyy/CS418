...
/** @global to store min and max */
var minmax = glMatrix.vec2.create();
...
/** @global the speed */
var speed = 1.0;
/** @global euler angle x for changing the pitch */
var eulerX = 0.0;
/** @global euler angle Z for changing the roll */
var eulerZ = 0.0;
/** @global a quaternion */
var ogquat = glMatrix.quat.create();
...
function setupShaders() {
  ...
  shaderProgram.uniformminh = gl.getUniformLocation(shaderProgram, "minh");
  shaderProgram.uniformmaxh = gl.getUniformLocation(shaderProgram, "maxh");
 ...
 * @param {Float32} alpha shininess cofficient
...
function setMaterialUniforms(alpha,a,d,s) {
  ...
  gl.uniform1f(shaderProgram.uniformminh, minmax[0]);
  gl.uniform1f(shaderProgram.uniformmaxh, minmax[1]);
  checkfog();
}

/**
 * Check if fog is checked
 */
function checkfog() {
    if (document.getElementById("fog").checked) {
        gl.uniform1i(shaderProgram.uniformfog, true);
    } else {
        gl.uniform1i(shaderProgram.uniformfog, false);
    }
}
//-------------------------------------------------------------------------
/**
...
function setupBuffers() {
    myTerrain = new Terrain(150,-1.0,1.0,-1.0,0.5);
...
function draw() { 
    ...
    glMatrix.mat4.rotateX(mvMatrix, mvMatrix, degToRad(-60));
//    glMatrix.mat4.rotateZ(mvMatrix, mvMatrix, degToRad(-60));
    setMatrixUniforms();
 ...
 * a. Q2  is set using fromEuler(Q2,x,y,z)
 * b. Q1 is set using multiply(Q1,Q2,Q1)
 * c. Transfrom original up and view direction vectors using transformQuat with Q1
 */
function animate() {
    var tmpquat = glMatrix.quat.create();
    ...
    if (currentlyPressedKeys["ArrowUp"]) {
        ...
        eyePt[2] -= speed*0.001;
        eulerX += 1;
        glMatrix.quat.fromEuler(tmpquat, eulerX*speed*0.0001, 0.0, 0.0);
        glMatrix.quat.multiply(ogquat, tmpquat, ogquat);
        glMatrix.vec3.transformQuat(viewDir, viewDir, ogquat); 
        glMatrix.vec3.transformQuat(up, up, ogquat);
        
//        glMatrix.quat.setAxisAngle(ogquat, viewDir, 1);
//        glMatrix.vec3.transformQuat(viewDir, viewDir, tmpquat);
//        glMatrix.vec3.transformQuat(up, up, tmpquat);
    }
...
//        eyePt[2] += 0.1*speed;
        eulerZ -= 1;
        glMatrix.quat.fromEuler(tmpquat, 0.0, 0.0, eulerZ*speed*0.00001);
        glMatrix.quat.multiply(ogquat, tmpquat, ogquat);
        glMatrix.vec3.transformQuat(viewDir, viewDir, ogquat); 
        glMatrix.vec3.transformQuat(up, up, ogquat);
    }
